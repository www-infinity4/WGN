(function () {
  ...
  let player=null, playerReady=false, apiRequested=false, entered=false, loadedKey="", loadedProgramVideoId="";
  ...
  function buildSchedule(nowMs){return engine.createDaySchedule(nowMs,programs,template);}
  ...
  function artFor(item){
    ...
    if(item.videoId)return `https://i.ytimg.com/vi/${item.videoId}/maxresdefault.jpg`;
    return "assets/channel-share.png";
  }
  function setProgramArt(item){
    document.body.style.setProperty("--program-hue",programHue(item));
    document.body.style.setProperty("--program-art",`url("${artFor(item)}")`);
  }
  function renderGuide(){
    ...
    els.guide.innerHTML=schedule.map(item=>{
      const movie=item.startsAtMs===schedule.find(x=>new Date(x.startsAtMs).getTime()===item.startsAtMs)?.startsAtMs && item.movie.collection.includes("PM") ? " · MOVIE" : "";
      return `<article class="guide-row ${item.startsAtMs>=schedule[0].startsAtMs+18*3600000?"movie-row":""}" data-id="${item.id}">
        <time>${formatStationTime(item.startsAtMs)}</time><strong>${item.movie.title}</strong>
        <span>${item.movie.year} · ${item.movie.collection}${movie}</span></article>`;
    }).join("");
  }
  function renderNext(currentBlock){
    ...
    els.next.innerHTML=[1,2,3].map(step=>{
      ...
      return `<article class="next-card" style="--card-hue:${programHue(item.movie)};--card-art:url('${art}')">
        <time>${formatStationTime(item.startsAtMs)}</time><div><h3>${item.movie.title}</h3><p>${item.movie.collection}</p></div></article>`;
    }).join("");
  }
  function showStationCard(state){
    ...
    els.cardCountdown.textContent=state.segment.cleared
      ? `${formatDuration(state.segmentRemaining)} until the next program`
      : "Guide slot ready · official embeddable source not connected";
  }
  ...
  function enterStation(){entered=true;els.enter.hidden=true;loadYouTubeApi();tick();}
  function loadYouTubeApi(){
    ...
    tag.referrerPolicy="strict-origin-when-cross-origin";document.head.appendChild(tag);
  }
  function startOver(){
  ...
  function rewind(){
    mode="timeshift";timeShiftBaseMs=activeClockMs()-30000;timeShiftStartedMs=Date.now();loadedKey="";tick();
  }
  ...
  function localShareCredit(reference){
    ...
    profile.tokens=Math.max(0,Number(profile.tokens)||0);profile.shareCount=Math.max(0,Number(profile.shareCount)||0)+1;
  ...
  async function shareChannel(){
    ...
    try{
      await navigator.share(share);const result=localShareCredit(share.url);
      els.shareStatus.textContent=result.awarded?"Shared · 1 StarCoin completed!":`Shared · StarCoin progress ${result.progressToNextCoin}/10`;
    }catch(error){if(!error||error.name!=="AbortError")els.shareStatus.textContent="Share did not complete.";}
  }

  window.onYouTubeIframeAPIReady=function(){
    player=new YT.Player("player",{width:"100%",height:"100%",playerVars:{playsinline:1,controls:1,enablejsapi:1,origin:location.origin,widget_referrer:location.href},
      events:{onReady:()=>{playerReady=true;player.unMute();player.setVolume(100);tick();},
        onError:()=>{if(loadedProgramVideoId)failedVideoIds.add(loadedProgramVideoId);loadedKey="";loadedProgramVideoId="";setTimeout(tick,250);}}});
  };
